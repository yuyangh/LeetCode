...
public:
	int oddEvenJumps(vector<int> &A) {
		...
		vector<bool> higher(A.size(), false), lower(A.size(), false);
		higher.back() = true;
		lower.back() = true;
		
		numIndexMap.emplace(A.back(), A.size() - 1);
		int result = 1;
		for (int i = A.size() - 2; i >= 0; --i) {
			// first element larger
			auto hi_it = numIndexMap.lower_bound(A[i]);
			// first key is not considered to go before k
			auto low_it = numIndexMap.upper_bound(A[i]);
			if (hi_it != numIndexMap.end()) {
				// check whether can reach from such low to the end
				higher[i] = lower[hi_it->second];
			}
			if (low_it != numIndexMap.begin()) {
				lower[i] = higher[(--low_it)->second];
			}
			if (higher[i]) {
				result++;
				cout << i << endl;
			}
			// use [] instead of emplace because emplace will not insert if value is already there
			numIndexMap[A[i]] = i;
		}
		return result;
	}
