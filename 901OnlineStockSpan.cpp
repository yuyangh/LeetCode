//
// Created by Yuyang Huang on 5/19/20.
//
...
// @lc code=start
/**
 * Since the numbers are in increasing order,
 * so we only need to store elements that are larger than current
 */
class StockSpanner {
public:
	StockSpanner() {
	...
	int next(int price) {
		int num=1;
		while (!pile.empty() && pile.top().first <= price) {
			num += pile.top().second;
			pile.pop();
		}
		pile.emplace(price,num);
		return num;
	}

private:
	// a stack of num and count
	std::stack<pair<int,int>> pile;
};
