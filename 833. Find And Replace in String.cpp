//
// Created by Yuyang Huang on 9/30/21.
//
...
public:
	...
	string findReplaceString(string s, vector<int> &indices, vector<string> &sources, vector<string> &targets) {
		unordered_map<int, pair<string, string>> indexReplacement;
		for (size_t i = 0; i < indices.size(); i++) {
			indexReplacement[indices[i]] = make_pair(sources[i], targets[i]);
		}
		
		string result;
		for (size_t i = 0; i < s.size(); i++) {
			auto it = indexReplacement.find(i);
			
			// if no index match
			if (it == indexReplacement.end()) {
				result.push_back(s[i]);
			} else {
				// if index match
				string source = it->second.first;
				string target = it->second.second;
				string original = s.substr(i, source.size());
				// if original string match the source string
				if (original != source) {
					result += original;
				} else {
					result += target;
